...
def importar_ml_view():
    if request.method == "POST":
        ...
        try:
            ...
            # Run worker in a separate process to avoid SQLite thread issues and worker timeouts
            log_path = os.path.join(app.config["UPLOAD_FOLDER"], f"import_worker_{lote_id.replace(':','-')}.log")
            env = os.environ.copy()
            env["UPLOAD_FOLDER"] = app.config.get("UPLOAD_FOLDER", "uploads")
            os.makedirs(app.config["UPLOAD_FOLDER"], exist_ok=True)
            # Launch worker and keep stdout/stderr for debugging
            with open(log_path, "a", encoding="utf-8") as logf:
                subprocess.Popen(
                    [python_exec, script_path, caminho, lote_id],
